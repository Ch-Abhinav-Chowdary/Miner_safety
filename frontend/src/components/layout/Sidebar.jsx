...
const Sidebar = ({ userRole, isOpen, onClose }) => {
  ...
  const navItems = [
    { to: '/dashboard', labelKey: 'nav.dashboard', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', roles: ['worker', 'supervisor', 'admin', 'dgms_officer'] },
    { to: '/profile', labelKey: 'nav.profile', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', roles: ['worker', 'supervisor', 'admin', 'dgms_officer'] },
    { to: '/gas-detection', labelKey: 'nav.gasDetection', icon: 'M13 10V3L4 14h7v7l9-11h-7z', roles: ['worker', 'supervisor', 'dgms_officer'] },
    ...
    { to: '/daily-checklist', labelKey: 'nav.safetyChecklist', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4', roles: ['worker', 'supervisor'] },
    { to: '/video-library', labelKey: 'nav.videoLibrary', icon: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', roles: ['worker', 'supervisor', 'dgms_officer'] },
    { to: '/hazard-reporting', labelKey: 'nav.reportHazard', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', roles: ['worker', 'supervisor', 'dgms_officer'] },
    { to: '/incident-library', labelKey: 'nav.incidentLibrary', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', roles: ['worker', 'supervisor', 'admin', 'dgms_officer'] },
    { to: '/case-studies', labelKey: 'nav.caseStudies', icon: 'M4 6h16M4 12h16M4 18h16M8 6v12m8-12v12', roles: ['worker', 'supervisor', 'admin', 'dgms_officer'] },
    ...
    // Admin Pages
    { to: '/admin/user-management', labelKey: 'nav.userManagement', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', roles: ['admin'] },
    { to: '/admin/reports', labelKey: 'nav.reports', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', roles: ['admin'] },
    { to: '/admin/sos-alerts', labelKey: 'nav.sosAlerts', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', roles: ['admin'] },
  ];

  // Filter navigation items based on user role
  const filteredNavItems = navItems.filter(item => 
    item.roles.includes(userRole || 'worker')
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-y-auto flex-grow py-6 px-4 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
          ...
          {filteredNavItems.map((item, index) => (
            ...
            >
              <NavLink
                to={item.to}
                className={({ isActive }) =>
                  `group relative flex items-center space-x-4 py-3.5 px-4 rounded-xl transition-all duration-300 overflow-hidden ${
                    isActive 
