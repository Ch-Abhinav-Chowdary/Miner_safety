...
const isPrivilegedRole = (role) =>
  ['supervisor', 'admin', 'dgms_officer'].includes(role || 'worker');
...
const formatDetailView = (doc, role = 'worker') => {
  const base = doc.toObject({ versionKey: false });
  const workerChecklist = base.preventiveChecklist?.filter((item) => item.role !== 'supervisor') || [];
  const supervisorChecklist = base.preventiveChecklist?.filter((item) => item.role !== 'worker') || [];
...
export const getCaseStudy = async (req, res) => {
  try {
    const role = req.query.role || req.user?.role || 'worker';
